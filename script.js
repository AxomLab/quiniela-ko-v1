...
function preloadAvatars(rows) {
  const uniqueAvatars = [...new Set(rows.map(p => String(p.avatar || '').trim()).filter(Boolean))];
...
function renderCarril(p, maxPuntos) {
  const progreso = p.puntos > 0
    ? Math.max(4, Math.min(96, (p.puntos / maxPuntos) * 92 + 4))
    : 0;
...
function renderMiniAvatar(p) {
  const hasAvatar = Boolean(p.avatar);
  const img = hasAvatar
    ? `<img src="${escapeAttr(avatarUrl(p.avatar))}" data-avatar-name="${escapeAttr(p.avatar)}" data-retry="0" alt="${escapeAttr(p.alias)}" decoding="async"
         onload="avatarLoaded(this)"
         onerror="avatarFailed(this)" />`
    : '';
